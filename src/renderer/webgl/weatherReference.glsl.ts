/**
 * SDR GLSL reference for the retired WebGL2 weather path.
 *
 * Not a live backend — `createStreetViewRenderer` never constructs a GL weather
 * class. Keep must-match literals (rain darken, haze, fall Y) aligned with
 * weather-post.wgsl via `webglLookParity.test.ts`. See docs/RENDERER_FALLBACK.md.
 */

export const WEBGL_WEATHER_VERTEX_GLSL = `#version 300 es
...
export const WEBGL_WEATHER_FRAGMENT_GLSL = `#version 300 es
precision highp float;
