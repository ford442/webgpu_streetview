...
import { createMaterials, resetGlowRegistry } from './interior/MaterialFactory';
...
export class CarInterior {
    ...
    private buildInteriorFromBuilder(): void {
        ...
        // Sun shafts (volumetric god rays at low sun angles)
        ...
        // Live center infotainment display
        ...
        // Detail props (cup, vanity, micro interactions)
        if (this.quality !== 'low') {
        ...
        // Point animator at (possibly new) gauge rig after build/vehicle swap
        this.animator?.setGaugeRig(this.gaugeRig);
    }
    ...
    public setVehicleType(vehicleType: VehicleType): void {
        ...
        const mats = createMaterials(this.vehicleConfig, this.gpuProfile);
        this.dashboardMaterial = mats.dashboard;
        this.chromeMaterial = (mats as any).chrome;
        this.leatherMaterial = mats.leather;
        this.metalMaterial = mats.metal;
        this.frameMaterial = mats.frame;
        this.glassMaterial = mats.glass;
        this.mirrorMaterial = mats.mirror;
        this.accentMaterial = mats.accent;
        this.chromeMaterial = (mats as any).chrome ?? this.chromeMaterial;
        this.buildInteriorFromBuilder();
        this.setupWindowWeatherOverlay();
    ...
    public setWeatherIntensity(rain: number): void {
        ...
        // also drive animator ambient rain for droplets etc
        const rainNorm = rain;
        this.animator?.setAmbientState({ rainNorm });
    }
    ...
    public cycleDisplayPage(): string | null {
        const next = this.centerDisplay?.cyclePage();
        return next ?? null;
    }
