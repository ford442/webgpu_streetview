...
> **Archived.** Do not follow this as a live contract. Rearview here still describes sampling the **forward** Street View canvas — that fake was removed in #171. Live: `src/car/RearviewMirror.ts` + billing-gated `src/car/rearViewFeed.ts`. Composition is `AppProviders` + `AppShell`, not `App.tsx`. See [`AGENTS.md`](../../AGENTS.md).
